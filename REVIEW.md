...
5. ~~**kgo's internal logger isn't wired.**~~ **Fixed** (commits `cca512f`, then `kslog` swap): wired via `kgo.WithLogger(kslog.New(logger))` using the official `github.com/twmb/franz-go/plugin/kslog` adapter.
